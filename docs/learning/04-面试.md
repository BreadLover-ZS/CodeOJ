...
本文用于完成[模块篇](02-模块/02-公共层与用户服务.md)与[链路篇](03-链路.md)后的面试复习。每题先自己回答，再看参考答案。回答时按“当前实现 → 问题证据 → 二开方向 → 验证范围”组织，避免把设计说成已实现。

这是一份问答材料；源码阅读顺序和闭卷成果测试统一放在[链路篇](03-链路.md)。
...
**参考回答：** 数据库是业务事实来源，消息只传 ID，消息体更小，消费者可重新读取任务状态作为幂等判断依据。当前的“先查 WAITING、再普通更新”不是原子抢占，不能据此声称已防止并发重复判题。代价是消费时重新读取题目和用例；如果题目被修改，旧提交可能按新用例执行，因此需要题目版本或不可变快照。
...
### 20. 为什么 `UserFeignClient#getLoginUser` 不算一次远程调用？

**参考回答：** 这个方法定义为接口的 `default` 方法，直接从当前请求的 Session 取用户；`isAdmin` 也是本地方法。真正的 Feign HTTP 方法有 `@GetMapping` 等映射，例如按 ID 查询用户。画调用链时必须区分“调用 Feign 接口对象”与“发起跨服务请求”。

**源码依据：** `UserFeignClient#getLoginUser`、`QuestionController#doQuestionSubmit`。

**继续追问：** 题目服务为什么也能读到登录 Session？内部 Feign 调用如何带密钥？

### 21. 提交成功后，前端怎样获得判题结果？

**参考回答：** 做题页提交成功后显示提示并跳转到提交记录页；当前页面没有用返回的提交 ID 做单条详情追踪。提交记录页查询列表，当前页有 WAITING/RUNNING 记录时每 3 秒刷新一次，全部为终态后停止。这只是前端观察机制，不保证每个任务都能到终态。

**源码依据：** `ViewQuestionView.vue#doSubmit`、`QuestionSubmitView.vue#refreshJudgingStatus`。

**继续追问：** 如果任务永久 WAITING，轮询会怎样？列表翻页后能否一直追踪同一 ID？

### 22. 代码中有 5 秒运行超时，为什么还可能卡住？

**参考回答：** Java 沙箱先同步读取子进程 stdout，之后才调用 `waitFor(runTimeoutMs)`。若进程不结束也不输出，读取可能一直等待 EOF，执行不到带超时的等待；输出长度截断也不是子进程资源隔离。需要把读流与整体截止时间统一管理，并用故障用例验证。

**源码依据：** `JavaNativeCodeSandbox#executeCode`、`readOutput`。

**继续追问：** stderr 填满管道怎么办？超时后如何回收子进程树？

**回答边界：** 当前是源码分析，不是已完成超时修复。

### 23. 服务启动了，能否证明 RabbitMQ 拓扑已创建？

**参考回答：** 不能。`InitRabbitMqBean` 创建原生连接时只设置了 host，未读取 Compose 中的 RabbitMQ 用户名和密码；异常只写日志。使用非 guest 凭据时必须实测交换机、队列、绑定和一次真实消息的发布消费，容器启动状态不是这些结果的证据。

**源码依据：** `InitRabbitMqBean#init`、`docker-compose.service.yml` 的 RabbitMQ 环境变量。

**继续追问：** 生产者发送调用成功，是否就证明消息可路由且已被处理？
