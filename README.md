...
| [01 总览](docs/learning/01-总览.md) | 项目地图、四阶段阅读顺序和完成标准 | 第一次接触项目时 |
| [02 模块](docs/learning/02-模块/02-公共层与用户服务.md) | 逐模块理解前端、公共层与用户、网关与题目、判题与 MQ、沙箱、部署；各篇末尾有关键问题 | 正式读跨服务链路前；完整索引见总览 |
| [03 链路](docs/learning/03-链路.md) | 4 条必读判题路径、1 条选读鉴权路径及闭卷成果测试 | 已能解释各模块职责后 |
| [04 面试](docs/learning/04-面试.md) | 源码依据、继续追问、回答边界和个人贡献口径 | 完成链路测试后 |
| [前端说明](codeoj-frontend/README.md) | 前端目录、命令、API 客户端和联调检查 | 实际开发或联调前端时 |

学习按“总览 → 模块 → 链路 → 面试”推进。模块阶段先读关键入口和职责，链路阶段再跨服务画正常与故障时间线；测试不通过时，回到对应方法核对，不做全仓库通读。
...
当前源码对编译错误、运行错误和超时的返回契约存在缺陷，以上第 4 项预计不会全部通过；详见[链路篇](docs/learning/03-链路.md)。
...
完成顺序和面试口径见[总览](docs/learning/01-总览.md)、[链路篇](docs/learning/03-链路.md)与[面试篇](docs/learning/04-面试.md)。压测必须分开报告“提交 API 吞吐”和“真实判题吞吐”，并保存环境、脚本、原始结果和失败拐点。
