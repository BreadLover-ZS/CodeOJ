...
| [项目导览](docs/learning/00-project-overview.md) | 模块协作、提交状态和沙箱/判题设计的简要说明 | 正式读源码前 |
...
面试准备按“项目导览 → 源码阅读路径 → 阅读成果测试 → 面试问题”推进。第一轮跳过前端页面、普通 CRUD、通用工具类和配置项细节；测试不通过时，只回到对应路径中的方法，不做全仓库通读。
